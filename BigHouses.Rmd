...
plot(seamap, main='A Basic Seattle Map')
```
...
That doesn't make too much sense with no basemap, and we still need to subset to Seattle. To subset, we use the `%over%` operator.
...
plot(seamap, main='Seattle...Now With Houses!')
plot(hs.sea, add=T, pch=20)
```
...
If it was a projection error, the West side shoreline and North and South boundaries would look worse.
